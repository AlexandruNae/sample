$connString = "Data Source=192.168.0.58\mediscmurepl;Database=MedisRepl;User ID=nexus;Password=U&e%3QY=zSZv8ZKQ"
$password ='U&e%3QY=zSZv8ZKQ'
$username = 'nexus'
$server = '192.168.0.58\mediscmurepl'
$database = 'MedisRepl'
...
foreach ($f in $files){
    ...
    $ds_firma =  Invoke-Sqlcmd -ServerInstance $server -Database $database -Username $username -Password $password -Query $searchSQL

    $id_firma = $ds_firma.id_firma
    $cod_fiscal = $ds_firma.cod_fiscal
    $denumire_firma = $ds_firma.denumire
    ...
    $ds_medic =  Invoke-Sqlcmd -ServerInstance $server -Database $database -Username $username -Password $password -Query $searchSQL

    Write-Output("------------------------------------------------------------------------------------------"+$ds_medic.activ)
    $IBAN = @()
    $IBAN = $ds_medic.ent_juridica[0]
    if($IBAN.Length -eq 1){
        $IBAN = $ds_medic.ent_juridica
    }
    $IBAN = $IBAN -replace '\s',''
    $IBAN = $IBAN -replace ' ',''
    ...
    if(!$ds_medic.medic){   
        ...
        $ds_extern =  Invoke-Sqlcmd -ServerInstance $server -Database $database -Username $username -Password $password -Query $searchSQL


        $id_medic = $ds_extern.id_medicext
        ...
        if($id_medic.Length -gt 1){
            for($i=0;$i -lt $id_medic.Length;$i++){
                ...
                $denumire_medic = $ds_extern.nume[$i] +" "+ $ds_extern.prenume[$i] 

                $cod = $ds_extern.cod_parafa

                $reprezentant_hr = $ds_extern.reprezentant_hr[$i]
            ...
            $denumire_medic = $ds_extern.nume +" "+ $ds_extern.prenume
            $cod = $ds_extern.cod_parafa
            Write-Output("222222222222 EXTERN SI SA VEDEM NUMELE: "+$denumire_medic)
            $reprezentant_hr = $ds_extern.reprezentant_hr[0]
            $id_forma_contr = "Extern"
            $medic_extern = $true
        ...
        for($m=0;$m -lt $all_medics.length; $m++){
            ...
            $verif = [string]$ds_medic.activ[$m]
            $denumire_medic = $ds_medic.denumire[$m]
            if($all_medics.length -eq 1){
                $denumire_medic = $ds_medic.denumire
            }
            ...
            $reprezentant_hr = $ds_medic.reprezentant_hr[$m]
            ...
            if($id_medic){
                ...
                if($id_medic.Length -gt 1){
                    for($i=0;$i -lt $id_medic.Length;$i++){
                        $IBAN = $ds_extern.cont[$i]
                        $IBAN = $IBAN -replace '\s',''
                        $IBAN = $IBAN -replace ' ',''

                        $denumire_medic = $ds_extern.nume[$i] +" "+ $ds_extern.prenume[$i]
                        $cod = $ds_extern.cod_parafa[$i]
                        $reprezentant_hr = $ds_extern.reprezentant_hr[$i]
                        $id_forma_contr = "Extern"
                        $medic_extern = $true
                        $IBAN = $ds_extern.cont[$i]
   ...
   # }
    ...
    for($i=0; $i -lt $medici_din_clasa.Length; $i++){
        ...
        if($medici_din_clasa[$i].activ -eq $true){
            if($medici_din_clasa[$i].intern_extern -eq 'Intern'){
                ...
                $ds_intern =  Invoke-Sqlcmd -ServerInstance $server -Database $database -Username $username -Password $password -Query $searchSQL
            ...
            for($top_2 = 0; $top_2 -lt 2; $top_2++){
                $an_factura = $ds_intern.an[$top_2]
                
                $luna_factura = $ds_intern.luna[$top_2]

                $an_factura = [int]$an_factura
                $luna_factura = [int]$luna_factura
                $today =  [datetime]::parseexact($today, 'dd/MM/yyyy', $null)
                Write-Output("sume: "+$ds_intern.valoare[$top_2])
                Write-Output("---")
                if($an_factura -eq $today.Year -and $luna_factura -eq ($today.AddMonths(-1).Month) -or $an_factura -eq ($today.Year-1) -and $luna_factura -eq ($today.AddMonths(-1).Month)){
                    if($top_2 -eq 0){
                        $medici_din_clasa[$i].suma_actuala = $ds_intern.valoare[$top_2]
                        $an_sikulix = $an_factura
                        $luna_sikulix = $luna_factura
                    }else{
                        $medici_din_clasa[$i].suma_luna_trecuta = $ds_intern.valoare[$top_2]
                        $an_sikulix = $an_factura
                        $luna_sikulix = $luna_factura
                    }
            }
    ...
    for($i=0; $i -lt $medici_din_clasa.Length; $i++){
        ...
        if($medici_din_clasa[$i].verif_cmr_malpraxis -eq $true -and $medici_din_clasa[$i].intern_extern -eq "Intern"){
            ...
            $ds_ext =  Invoke-Sqlcmd -ServerInstance $server -Database $database -Username $username -Password $password -Query $searchSQL


            $date_expirare = @()
            $emitent = @()
            for($p=0; $p -lt $ds_ext.data_expirare.length; $p++){
                
                $date_expirare +=$ds_ext.data_expirare[$p]
                $emitent += $ds_ext.emitent[$p]
...
for($j=0; $j -lt $array_reprezentanti.Length; $j++){
        ...
        $ds_hr =  Invoke-Sqlcmd -ServerInstance $server -Database $database -Username $username -Password $password -Query $searchSQL

        Write-Output("nume repre : "+$ds_hr.nume)
        $nume_reprezentant += $ds_hr.nume +' '+ $ds_hr.prenume
